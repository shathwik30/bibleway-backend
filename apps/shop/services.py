...
class PurchaseService(BaseService[Purchase]):
    ...
    def verify_purchase(
        ...
        *,
        ...
        # Validate the receipt with the appropriate store BEFORE marking as validated
        try:
        ...
        # Receipt validated successfully; create the purchase record
        try:
            ...
            # Race condition: another request with the same transaction_id
            # was processed between our exists() check and the create().
            raise ConflictError(
                detail=f"Transaction '{transaction_id}' has already been processed."
            )
