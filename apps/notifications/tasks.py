...
@shared_task(
    bind=True, max_retries=3, default_retry_delay=60, soft_time_limit=60, time_limit=120
)
def send_push_notification(self, user_id, title, body, data=None):
    ...
    try:
        ...
        tokens = list(
            DevicePushToken.objects.filter(user_id=user_id, is_active=True).values_list(
                "token", flat=True
            )
        )
        ...
        if e.response is not None and 400 <= e.response.status_code < 500:
            logger.error(
                "Push notification permanent failure (%s): %s",
