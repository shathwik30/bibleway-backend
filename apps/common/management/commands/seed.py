...
class Command(BaseCommand):
    ...
    def _flush(self):
        ...
        app_tables = [
            "analytics_boostanalyticsnapshot",
            "analytics_postboost",
            "analytics_postview",
            "notifications_notification",
            "notifications_devicepushtoken",
            "social_reply",
            "social_report",
            "social_reaction",
            "social_comment",
            ...
            "shop_purchase",
            "shop_product",
            "bible_segregatedpagecomment",
            "bible_segregatedpagelike",
            "bible_translatedpagecache",
            "bible_note",
            "bible_highlight",
            "bible_bookmark",
            "bible_segregatedpage",
            "bible_segregatedchapter",
            "bible_segregatedsection",
            "verse_of_day_verseofday",
            "verse_of_day_versefallbackpool",
            "admin_panel_adminlog",
            "admin_panel_adminrole",
            "admin_panel_boosttier",
            "accounts_otptoken",
            "accounts_blockrelationship",
            "accounts_followrelationship",
            "accounts_user",
    ...
    def _seed_accounts(self):
        ...
        user_data = [
            {
                "email": "testuser@bibleway.io",
                "full_name": "Test User",
                "dob": date(1995, 6, 15),
                "gender": "male",
                "country": "United States",
                "lang": "en",
                "bio": "A test account for development and QA.",
            },
            {
                "email": "jane@bibleway.io",
            ...
            {
                "email": "carlos@bibleway.io",
                "full_name": "Carlos Rivera",
                "dob": date(1988, 11, 5),
                "gender": "male",
                "country": "Brazil",
                "lang": "es",
                "bio": "Youth pastor. Bible nerd.",
            },
            {
                "email": "grace@bibleway.io",
                "full_name": "Grace Kim",
                "dob": date(2000, 7, 12),
                "gender": "female",
                "country": "South Korea",
                "lang": "ko",
                "bio": "New believer hungry to learn.",
            },
            {
                "email": "admin@bibleway.io",
        ...
        FollowRelationship.objects.bulk_create(
            [FollowRelationship(follower=f, following=t) for f, t in follows]
        )
    ...
    def _seed_bible(self, users):
        ...
        Note.objects.create(
            ...
            text="This verse gives me so much comfort.",
        )
        Note.objects.create(
            user=users[1],
            note_type="segregated",
            content_type=page_ct,
            object_id=pages[5].id,
        ...
        SegregatedPageComment.objects.create(
            user=users[1],
            page=pages[0],
            content="Beautiful story of creation!",
        )
        SegregatedPageComment.objects.create(
            user=users[2],
            page=pages[4],
            content="The beatitudes are life-changing.",
        )
        ...
        Notification.objects.bulk_create(
            [
                ...
                Notification(
                    recipient=users[3],
                    sender=users[1],
                    notification_type="prayer_comment",
    ...
    def _seed_analytics(self, users, posts, prayers):
        ...
        for post in posts:
            for viewer in users[:3]:
                views.append(
                    PostView(content_type=post_ct, object_id=post.id, viewer=viewer)
                )

        for prayer in prayers:
            for viewer in users[:2]:
                views.append(
                    PostView(content_type=prayer_ct, object_id=prayer.id, viewer=viewer)
                )
