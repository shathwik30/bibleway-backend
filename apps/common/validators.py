...
# Maps content categories to a list of (offset, magic_bytes) tuples.
...
_VIDEO_MAGIC: list[tuple[int, bytes]] = [
    (4, b"ftyp"),                   # MP4 / MOV (ISO BMFF)
    (0, b"\x00\x00\x00"),          # MP4 variant
]
_AUDIO_MAGIC: list[tuple[int, bytes]] = [
    (0, b"\xff\xfb"),              # MP3 (MPEG frame sync)
    (0, b"\xff\xf3"),              # MP3 variant
    (0, b"\xff\xf2"),              # MP3 variant
    (0, b"ID3"),                    # MP3 with ID3 tag
    (0, b"OggS"),                   # OGG
    (0, b"RIFF"),                   # WAV (RIFF container)
    (4, b"ftyp"),                   # M4A / AAC (ISO BMFF)
]
...
def validate_voice_note_file(file: UploadedFile) -> None:
    ...
    if not _verify_magic_bytes(file, _AUDIO_MAGIC):
        raise ValidationError(
            "File content does not match a valid audio format."
        )
    content_type = getattr(file, "content_type", "")
