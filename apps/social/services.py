...
def _validate_object_exists(content_type: ContentType, object_id: UUID) -> None:
    ...
    if model_class is None:
        raise NotFoundError(detail=f"Unknown content type '{content_type.model}'.")
...
def _check_block_for_content(
    user: User, content_type: ContentType, object_id: UUID
) -> None:
...
class PostService(BaseService[Post]):
    ...
    def get_by_id_for_user(self, pk: UUID, *, requesting_user: User) -> Post:
        ...
        try:
            ...
            raise NotFoundError(detail=f"Post with id '{pk}' not found.")
...
class PrayerService(BaseService[Prayer]):
    ...
    def get_by_id_for_user(self, pk: UUID, *, requesting_user: User) -> Prayer:
        ...
        try:
            ...
            raise NotFoundError(detail=f"Prayer with id '{pk}' not found.")
...
class ReplyService(BaseService[Reply]):
    ...
    def create_reply(
        ...
        *,
        ...
        try:
            comment = Comment.objects.select_related("content_type", "user").get(
                pk=comment_id
            )
